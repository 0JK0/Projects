...
def slide():
    while running == True:
        ...
        if keys[pygame.K_LEFT]: dire[0] -= 1
...
while running:
   ...
   #If something happens and that something is a key being pressed do:     
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_LEFT: dire [0] -= 1
        elif event.key == pygame.K_RIGHT: dire [0] += 1
        elif event.key == pygame.K_UP: dire [1] -= 1
        elif event.key == pygame.K_DOWN: dire [1] += 1
